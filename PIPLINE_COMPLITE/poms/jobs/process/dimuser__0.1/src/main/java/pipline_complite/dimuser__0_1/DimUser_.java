...
package pipline_complite.dimuser__0_1;
 ...
 * @status
 */
public class DimUser_ implements TalendJob {
	...
	private final String jobName = "DimUser_";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(DimUser_.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_PIPLINE_COMPLITE_DimUser_ = new byte[0];
		static byte[] commonByteArray_PIPLINE_COMPLITE_DimUser_ = new byte[0];
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PIPLINE_COMPLITE_DimUser_) {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PIPLINE_COMPLITE_DimUser_.length) {
					if (length < 1024 && commonByteArray_PIPLINE_COMPLITE_DimUser_.length == 0) {
						commonByteArray_PIPLINE_COMPLITE_DimUser_ = new byte[1024];
					} else {
						commonByteArray_PIPLINE_COMPLITE_DimUser_ = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PIPLINE_COMPLITE_DimUser_, 0, length);
				strReturn = new String(commonByteArray_PIPLINE_COMPLITE_DimUser_, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PIPLINE_COMPLITE_DimUser_, 0, length);
				strReturn = new String(commonByteArray_PIPLINE_COMPLITE_DimUser_, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:tesb6hhob3RNMQ7SnuGmsZMl/C14rft9l9eMeghc3ke6tpU=");
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1 + "]([userId] INT )");
					}
				}
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1 + "] ([userId]) VALUES (?)";
								...
								if (columnIndexWithD_tFileInputDelimited_1 < rowtFileInputDelimited_1.length) {
									...
									if (rowtFileInputDelimited_1[columnIndexWithD_tFileInputDelimited_1].length() > 0) {
										try {
											...
											row1.userId = ParserUtils.parseTo_Integer(
...
// # Output table : 'DimUser'
								DimUser_tmp.userId = row1.userId;
								DimUser = DimUser_tmp;
// ###############################
...
// Start of branch "DimUser"
							if (DimUser != null) {
								...
								row2 = null;
								finder_tUniqRow_1.userId = DimUser.userId;
								finder_tUniqRow_1.hashCodeDirty = true;
								if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
									KeyStruct_tUniqRow_1 new_tUniqRow_1 = new KeyStruct_tUniqRow_1();

									new_tUniqRow_1.userId = DimUser.userId;
...
// Start of branch "row2"
								if (row2 != null) {
									...
									if (row2.userId == null) {
										...
										pstmt_tDBOutput_1.setInt(1, row2.userId);
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final DimUser_ DimUser_Class = new DimUser_();

		int exitCode = DimUser_Class.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = DimUser_.class.getClassLoader()
					.getResourceAsStream("pipline_complite/dimuser__0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = DimUser_.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DimUser_");
		}
...
/************************************************************************************************
 * 74944 characters generated by Talend Open Studio for Data Integration on the
 * 9 janvier 2025, 20:11:39 CET
 ************************************************************************************************/